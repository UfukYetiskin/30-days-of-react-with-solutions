...
JavaScript, nesne yönelimli bir programlama dilidir. JavScript'teki 
her şey, özellikleri ve yöntemleriyle bir nesnedir. Bir nesne 
oluşturmak için sınıf oluşturuyoruz. Bir Sınıf, bir nesne oluşturucu
veya nesneler oluşturmak için bir "plan" gibidir. Bir nesne 
oluşturmak için bir sınıfı başlatıyoruz. Sınıf, nesnenin 
niteliklerini ve davranışını tanımlarken, nesne ise sınıfı temsil 
eder.

Bir sınıf oluşturduğumuzda, istediğimiz zaman ondan nesne 
oluşturabiliriz. Bir sınıftan nesne oluşturmaya sınıf somutlaştırma
denir.

Nesne bölümünde, bir nesne literalinin nasıl oluşturulacağını 
gördük. Nesne değişmezi bir singleton'dur. Benzer bir nesne elde 
etmek istiyorsak, onu yazmalıyız. Ancak, sınıf birçok nesne 
oluşturmaya izin verir. Bu, kod miktarını ve kod tekrarını azaltmaya 
yardımcı olur.
...
Gördüğünüz gibi bir kişi nesnesi oluşturduk. Sınıfın henüz herhangi bir özelliği 
olmadığı için nesne de boştur.

Sınıf için farklı özellikler iletmek için sınıf yapıcısını kullanalım.

-- Class Constructor---
 Constructor, nesnemiz için bir plan oluşturmamıza izin veren yerleşik bir işlevdir. Constructor 
işlevi, yapıcı bir anahtar sözcükle başlar ve ardından bir parantez gelir. Parantez 
içinde nesnenin özelliklerini parametre olarak iletiyoruz. Bu anahtar sözcüğü, yapıcı 
parametrelerini sınıfa eklemek için kullanırız .

Aşağıdaki Person sınıfı oluşturucu, firstName ve lastName özelliğine sahiptir. Bu 
özellikler, bu anahtar kelime kullanılarak Person sınıfına eklenir . Bu , sınıfın 
...
const ferat = new Person('Ferat', 'Yetişkin')
const gurbet = new Person('Gurbet', 'Yetişkin')
console.log(ferat) //Person {firstName: 'Ferat', lastName: 'Yetişkin'}
console.log(gurbet) //Person {firstName: 'Gurbet', lastName: 'Yetişkin'}
...
const vahap = new Person1('Abdulvahap', 'Yetişkin', '56')
console.log(vahap)//Person1 {firstName: 'Abdulvahap', lastName: 'Yetişkin', age: '56', country: 'Turkey'}
